...
#define DIST_MAX 11.f
#define TESS_MAX 10.f
#define TESS_MIN 1.f
...
CHS_OUT CHS(InputPatch<VS_OUT, 3> patch, uint patch_id : SV_PrimitiveID) {
    ...
    float dist = length(center.xyz - camera_pos);
    float tess = 11.f - dist;
    tess = clamp(tess, TESS_MIN, TESS_MAX);
...
float height(const OutputPatch<CPHS_OUT, 3> input, float r, float s, float t)
{
    ...
    return tex_height.SampleLevel(sam_lw, uv,0).x;
}
...
DS_OUT DS(CHS_OUT patch, float3 location : SV_DomainLocation, const OutputPatch<CPHS_OUT, 3> input){
    ...
    position.y = tex_height.SampleLevel(sam_lw, uv,0).x;
